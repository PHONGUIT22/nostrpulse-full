...
/**
 * Centralized encryption manager that handles all NIP-17/NIP-59 gift wrap operations.
 * Implements 3-layer envelope encryption: Rumor -> Seal (Kind 13) -> Gift Wrap (Kind 1059).
 */
...
export class EncryptionManager {
   ...
   * @param senderPrivateKey - Sender's private key in hex
   * @param recipientPublicKey - Recipient's public key in hex
   ...
   * @param recipientPrivateKey - Recipient's private key in hex
  ...
  public async decryptMessage(
    ...
    try {
      ...
      if (rumor.kind === PRIVATE_DIRECT_MESSAGE_KIND) {
        // If it's a kind 14, the content might be JSON-encoded message
        try {
